...
namespace MindCheck.Service
{
    public class ApiService
    {
        ...
        public async Task<List<FinalAnswerModel>> GetFinalAnswersByTestIdAsync(int testId)
        {
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    var finalAnswers = JsonConvert.DeserializeObject<List<FinalAnswerModel>>(json);
        ...
        public async Task<int> GetTestCountByCategoryAsync(string category)
        {
            try
            {
                var testItems = await GetTestItemsByCategoryAsync(category);
